...
VERSION_STRING = f'{Path(__file__).name} version 0.1.2'
...
def _update_domain_entry(domain_entry: DomainEntry, deps: Dependencies):
    ...
    if alias is None:
        ...
        log(f'creating new alias {new_alias.name} with ip {ip}')
        deps.alias_create(new_alias)
    else:
        if alias.cidr != ip:
            log(f'updating alias {alias.name} from {alias.cidr} to {ip}')
